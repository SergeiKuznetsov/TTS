...
//text = text.replace(/()()на (\d+[AB]*)(, \d+[AB]*)*: на/g, 'номер $3$4: на'); // row: 114 // съезд справа на 0: на М7, шоссе Энтузиастов, Центр
  ...
  //text = text.replace(/()()Берёзовский/g, ''); // row: 162 // Березо́вский - I do not know how to fix it. :-(
  text = text.replace(/()()Алабяно/g, 'Алабя́но'); // row: 163 // Ала́бяно-Балтийский тоннель
  text = text.replace(/()()(\d+)\s(([^\s]+\s){0,2})(улица|набережная|дорога|линия|аллея|площадь|просека|автодорога|эстакада|магистраль|дамба|хорда|коса|деревня|переправа|площадка|дорожка|трасса|Ферма|Авеню|Пристань|Рота|Слобода)( |$)/g, '$3-я $4$6$7'); // row: 164 // 3 Продольная магистраль -> 3-я Продольная магистраль
  text = text.replace(/()()(\d+)\s(([^\s]+\s){0,2})(проспект|переулок|проезд|тупик|бульвар|тракт|просек|взвоз|спуск|разъезд|переезд|квартал|путепровод|мост|сад|сквер|тоннель|туннель|парк|проток|канал|остров|микрорайон|район|городок|посёлок|поселок|вал|проулок|Дивизии|Бригады|Армии|Интернационал)( |$)/g, '$3-й $4$6$7'); // row: 165 // 1 проспект -> 1-й проспект
  text = text.replace(/()()(\d+)\s(([^\s]+\s){0,2})(шоссе|кольцо|село)( |$)/g, '$3-е $4$6$7'); // row: 166 // 1 шоссе -> 1-е шоссе
  text = text.replace(/()()(\d+)\s(Партсъезда|Съезда)( |$)/g, '$3-го $4$5'); // row: 167 // 20 Партсъезда -> 20-го Партсъезда
